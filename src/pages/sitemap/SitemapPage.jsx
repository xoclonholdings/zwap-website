...
const SITEMAP_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      // ✅ ADDED THIS (REQUIRED FOR GOOGLE COMPLIANCE)
      {
...
export default function SitemapPage({ onBack, onNavigate }) {
  ...
  return (
    <div style={{ minHeight: "100dvh", display: "flex", justifyContent: "center", background: "linear-gradient(180deg, #030308 0%, #080913 58%, #04050A 100%)", color: "#F5F7FF" }}>
      <div style={{ width: "100%", maxWidth: "960px", padding: "20px" }}>
        ...
        {SITEMAP_GROUPS.map((group) => (
          <div key={group.title} style={{ marginBottom: "20px" }}>
            ...
            {group.items.map((item) => (
              <div key={item.label}>
                <button onClick={() => handleItemClick(item)} style={{ width: "100%", textAlign: "left" }}>
                  {item.label}
                </button>
              </div>
            ))}
