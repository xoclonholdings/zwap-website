  ...
  if (activePage === "preview") {
    if (!previewUnlocked) {
      return (
        <>
          {/*
            Guarded preview route.
            We render nothing but reopen the modal safely on next tick.
          */}
          <RedirectToPreviewUnlock onOpen={() => openEarlyAccessModal("preview")} />
          <EarlyAccessModal
            isOpen={isModalOpen}
            onClose={closeEarlyAccessModal}
            email={email}
...
function RedirectToPreviewUnlock({ onOpen }) {
  useEffect(() => {
    onOpen();
  }, [onOpen]);
